...
def getAdmissionsInfo2019(soup):
    admDict = {}
    admissions = soup.find('table', class_ = "toccolours")    
    tables = admissions.find_all('tr')
    for table in tables:
        info = table.find_all('td')
        schoolName = info[0].text
        year = info[1].text
        admDict[schoolName] = year.strip("\n")
    admDict.pop("College/school")
    print(admDict)
    return admDict
